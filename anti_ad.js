...
const ifsm_helper = async() => {
  const which_page = (() => {
    ...
    if(!breadcrumb)
      return 'unknown';
    if(breadcrumb.innerText === '»Customers›Detail View› Sites List')
      return 'sites list';
    if(breadcrumb.innerText === '»Customers›Detail View›Sites List› Edit Site')
      return 'edit site';
    return 'unknown';
  })();

  if(which_page === 'sites list') {
    const table_rows = [...document.getElementById('ctl00_m_gvMain').children[0].children];
    table_rows.splice(0, 1);  // remove first element, which is the table headers
    console.log('Tommy ifsm etc');
    console.log(table_rows.length);

    for(const row of table_rows) {
      const site_name = row.children[0].innerText;

      if(site_name === 'AAA Window Rock Legislative Buildings')
        continue;
      if(site_name === 'Navajo Nation Office of Speaker')
        continue;
  ...
  } else if(which_page === 'edit site') {
    const site_name = document.getElementById('ctl00_m_addEditSite_txtName').value;
    const site_city = document.getElementById('ctl00_m_addEditSite_txtSiteCity').value;

    if(site_name === 'AAA Window Rock Legislative Buildings')
      throw new Error('No! Do not destroy this site!');
    if(site_name === 'Navajo Nation Office of Speaker')
      throw new Error('No! Do not destroy this site!');
    if(site_city !== '')
      throw new Error('Why is the site city not blank?');
    ...
    const delete_site = () => {
      ...
      script_tag.innerHTML = javascript_code;
      document.body.appendChild(script_tag);

      document.getElementById('ctl00_m_addEditSite_btnDelete').click();
    };
